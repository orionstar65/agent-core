...
#ifdef _WIN32
    int agent_pid = GetCurrentProcessId();
#else
...
void test_offender_identification() {
    ...
    // Add extension with high CPU usage that individually exceeds warn threshold
    // 50% CPU = 83.3% of 60% max (above 80% warn threshold)
    ...
    ext_usage.cpu_pct = 50.0;  // 83.3% of 60% max (above 80% threshold)
    ext_usage.mem_mb = 100;
    monitor->pid_usages_[2000] = ext_usage;
    
    // Total CPU: 80% which is 133% of 60% max -> Stop stage
    auto violation = enforcer.evaluate(config, monitor.get(), ext_manager.get());
