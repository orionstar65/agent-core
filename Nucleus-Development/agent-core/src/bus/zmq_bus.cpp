...
public:
    ZmqBusImpl(Logger* logger) : logger_(logger) {
        ...
        try {
            ...
            if (logger_) {
                logger_->log(LogLevel::Error, "Bus", "Failed to bind pub socket", {{"endpoint", pub_endpoint}, {"error", std::to_string(e.num())}});
            }
        ...
        try {
            ...
            if (logger_) {
                logger_->log(LogLevel::Error, "Bus", "Failed to connect req socket", {{"endpoint", req_endpoint}, {"error", std::to_string(e.num())}});
            }
        ...
        if (logger_) {
            ...
            fields["pub_endpoint"] = pub_endpoint;
            fields["req_endpoint"] = req_endpoint;
            logger_->log(LogLevel::Info, "Bus", "ZeroMQ bus initialized", fields);
        }
#else
        if (logger_) {
    ...
    void publish(const Envelope& envelope) override {
        ...
        if (logger_) {
            ...
            fields["correlationId"] = envelope.correlation_id;
            logger_->log(LogLevel::Debug, "Bus", "Published message", fields);
        }
#else
        if (logger_) {
            std::map<std::string, std::string> fields;
            fields["topic"] = envelope.topic;
            fields["correlationId"] = envelope.correlation_id;
            logger_->log(LogLevel::Debug, "Bus", "Published message (stub)", fields);
        }
#endif
    }
    ...
    void request(const Envelope& req, Envelope& reply) override {
        ...
        if (logger_) {
            ...
            fields["correlationId"] = req.correlation_id;
            fields["replyCorrelationId"] = reply.correlation_id;
            logger_->log(LogLevel::Debug, "Bus", "Request completed", fields);
        }
#else
        if (logger_) {
            std::map<std::string, std::string> fields;
            fields["topic"] = req.topic;
            fields["correlationId"] = req.correlation_id;
            logger_->log(LogLevel::Debug, "Bus", "Request (stub)", fields);
        }
    ...
    void subscribe(const std::string& topic,
        ...
        sub_thread_ = std::thread([this, topic]() {
            ...
            try {
                ...
                if (logger_) {
                    logger_->log(LogLevel::Error, "Bus", "Failed to connect sub socket", {{"endpoint", sub_endpoint}, {"error", std::to_string(e.num())}});
                }
                return;
            }
        ...
        if (logger_) {
            std::map<std::string, std::string> fields;
            fields["topic"] = topic;
            logger_->log(LogLevel::Info, "Bus", "Subscribed to topic", fields);
        }
#else
        if (logger_) {
            std::map<std::string, std::string> fields;
            fields["topic"] = topic;
            logger_->log(LogLevel::Info, "Bus", "Subscribed to topic (stub)", fields);
        }
        subscriptions_[topic] = callback;
#endif
    }
